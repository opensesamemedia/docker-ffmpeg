...
RUN apt-get install -y bear libsdl2-dev libopus-dev gnutls-dev


RUN \
  echo "**** grabbing ffmpeg ****" && \
  mkdir -p /tmp/ffmpeg && \
  git clone -b SyncStageDocker https://github.com/opensesamemedia/FFmpeg.git /tmp/ffmpeg 


RUN \
  echo "**** compiling ffmpeg ****" && \
  cd /tmp/ffmpeg && \
  ./config.sh -p && make -j 2
